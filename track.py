...
from yolov5.utils.general import LOGGER, check_img_size, increment_path, non_max_suppression, scale_coords, check_imshow, xyxy2xywh, increment_path
...
class Tracker:
    ...
    def __init__(self, config_path: str) -> None:
        ...
        self.df = pd.DataFrame(columns=["VehicleID", "Date", "Time", "Camera", "Speed", "Type"])
                        ...
                        for ID in copy.deepcopy(list_vehicles):
                            if (ID not in current_IDs):
                                ...
                                if (vehicle_infos[ID]['temporarily_disappear'] > 75) and \
                                    ...
                                    str_ID = str(ID)
                                    if opt.save_csv:
                                        df3 = pd.DataFrame([[str_ID, vehicle_infos[ID]['in_time'].strftime('%m/%d/%Y'), vehicle_infos[ID]['in_time'].strftime('%H:%M'), "cam1", 0, vehicle_infos[ID]['Type']]],
